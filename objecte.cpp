...
Capsa3D Objecte::calculCapsa3D()
{
    ...
    pmin = vec3(min[0],min[1],min[2]);
    pmax = vec3(max[0],max[1],max[2]);
    capsa.pmin = pmin;
...
{
    mat4 t1, t2;
    t1 = Translate(-(capsa.pmin[0]+capsa.a/2.0),-(capsa.pmin[1]+capsa.h/2),-(capsa.pmin[2]+capsa.p/2));
    t2 = Translate(capsa.pmin[0]+capsa.a/2.0,capsa.pmin[1]+capsa.h/2,capsa.pmin[2]+capsa.p/2);
    aplicaTGPoints(t2*m*t1);
}
...
void Objecte::toGPUSenseTextures(QGLShaderProgram *pr){
...
// Pintat en la GPU.
void Objecte::drawSenseTextures()
{
...
void Objecte::draw()
{
    ...
    glBufferSubData( GL_ARRAY_BUFFER, 0, sizeof(point4) * Index, &points[0] );
    glBufferSubData( GL_ARRAY_BUFFER, sizeof(point4) * Index, sizeof(color4) * Index, &colors[0] );
    glBufferSubData( GL_ARRAY_BUFFER, sizeof(point4) * Index + sizeof(color4) * Index,sizeof(vec2)*Index, &vertexsTextura[0]);
